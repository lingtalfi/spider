...
class ApplicationModule extends KamilleModule
{
    ...
    {
        if (true === XConfig::get("Core.useFastDevelopment")) {
            $this->disableHooks();
        }
    }


    protected function getSqlQueryByFile($database, $file)
    {
        $dbName = XConfig::get("Core.database");
        return str_replace($database, $dbName, file_get_contents($file));
    }
